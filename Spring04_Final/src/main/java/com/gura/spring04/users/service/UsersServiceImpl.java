...
public class UsersServiceImpl implements UsersService {
	...
	public void addUser(UsersDto dto) {
		...
		// CharSequence는 String의 interface이다. ==>> 그러므로 문자열은 CharSequence type으로도 받을 수
		// 있다.
		String encodedPwd = encoder.encode(pwd);
		// 암호화한pwd를 다시 dto에 넣는다.
		dto.setPwd(encodedPwd);

		// 그다음 dao처리하면
		dao.insert(dto);
		// 관리자 또한 회원의 비밀번호를 보지 못한다.
	}
	...
	public void loginProcess(UsersDto dto, HttpSession session) {
		...
		String pwd = resultDto.getPwd();

		// 만일 select 된 회원 정보가 존재하고
		if (resultDto != null) {
			// 비밀번호도 일치한다면 isValid 에 true 를 대입한다.
			// BCrypt클래스의 static 메소드를 이용해서 입력한 비밀번호화 암호화해서 저장된 비밀번호의 일치 여부를 알아내야 한다.
			isValid = BCrypt.checkpw(inputPwd, pwd);
			// BCrypt.checkpw(암호화 되지 않은 문자열, 암호화된 문자열); ->비밀번호 확인
		}
