...
<body>
	<div class="container">
		<!-- 만일 이전 글(과거 글)의 글 번호가 0이 아니라면 (이전 글이 존재한다면) -->
		<c:if test="${dto.prevNum ne 0 }">
			<a
				href="detail?num=${dto.prevNum }&condition=${condition}&keyword=${encodedK}">이전
				글</a>
		</c:if>
		<!-- 만일 다음 글(최신 글)의 글 번호가 0이 아니라면 (다음 글이 존재한다면) -->
		<c:if test="${dto.nextNum ne 0 }">
			<a
				href="detail?num=${dto.nextNum }&condition=${condition}&keyword=${encodedK}">다음
				글</a>
		</c:if>
		<h3>글 상세 보기</h3>
		<table>
		...
		<c:if test="${sessionScope.id eq dto.writer }">
			<a href="updateform?num=${dto.getNum() }">수정</a>
			<a href="javascript:" onclick="deleteConfirm()">삭제</a>
			<script>
            function deleteConfirm(){
               const isDelete=confirm("이 글을 삭제 하겠습니까?");
               if(isDelete){
                  location.href="delete?num=${dto.getNum()}";
               }
            }
          </script>
		</c:if>
